<?php
// filepath: c:\xampp\htdocs\dormitory-management-system\tenant\checkout_booking.php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['booking_id'])) {
    ...
    // Verify booking belongs to tenant and is checked_in
    ...
    if ($booking['status'] !== 'checked_in') {
        set_flash_message('Only checked-in bookings can be checked out', 'error');
        redirect(TENANT_URL . '/bookings');
    }
    
    // Check out the booking
    $stmt = $conn->prepare("UPDATE bookings SET status = 'checked_out', check_out_date = CURDATE(), updated_at = NOW() WHERE id = ?");
    ...
    if ($stmt->execute()) {
        ...
        if ($booking['is_bedspace_booking'] && $booking['bedspace_id']) {
            ...
            // Update room status to available if it was a full room booking
        ...
        set_flash_message('Successfully checked out from Room ' . $booking['room_number'] . '. Thank you for staying with us!', 'success');
    } else {
        set_flash_message('Failed to check out. Please contact support.', 'error');
    }
    $stmt->close();
